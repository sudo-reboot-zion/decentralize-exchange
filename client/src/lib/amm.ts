...
const AMM_CONTRACT_ADDRESS = "ST30KZS9YJ5BRTM39HMDC0QBF83PXNDSQTK4FFZWJ"
...
export async function addLiquidity(pool: Pool, amount0: number, amount1: number) {
    ...
    if (pool.liquidity > 0) {
        ...
        if (amount1 < idealAmount1) {
            throw new Error (
                `Cannot add liquidity in these amounts. You need to supply at least ${idealAmount1} ${ pool["token-1"].split(".")[1]} along with ${amount0} ${pool["token-0"].split(".")[1]}`
            );
        }
    }
